...
public class RepositoryResponse {
    ...
    public RepositoryResponse(Document xml, StatusLine statusLine, Header[] headers) {
        this.xml = xml;
     ...
     */
    public void checkStatus(int low, int high, String msg, Object ... args) throws ServerException {
        int statusCode=getStatusCode();
        if (statusCode>=low && statusCode<=high)
            return;
        throw new ServerException(msg,this,args);
    }
     ...
     */
    public void enforce200(String msg) throws ServerException {
        int statusCode=getStatusCode();
        if (statusCode!=200)
            throw new ServerException("HTTP Status 200 expected - %d returned : %s",this,statusCode, msg);
    }

    public void enforce200(Document xml) throws ServerException {
        try {
            enforce200(Util.xmlDocumentToString(xml));
        } catch (Exception e) {
            throw new ServerException("Unable to transform XML - " + e.getMessage());
        }
    }
    ...
    public String toString() {
        ...
        if (xml != null)
            try {
                sb.append(ServerConnector.prettyXml(xml)).append("\n");
            } catch (Exception e) {
